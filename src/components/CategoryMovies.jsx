...
const CategoryMovies = () => {
  ...
  const fetchCategory = async () => {
    const res = await axios.get(
      `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.REACT_APP_KEY}&with_genres=${genresId}&language=en-US&page=${page}`
    );
    setData(res?.data);
    ...
    <>
      ...
      <Box sx={{ mt: '-4rem' }}>
        {genress ? (
          ...
          <Titlemod title={'Category : Movies'} color="#dcdcdc" />
        )}
      </Box>
      <div style={{ width: '100%' }}></div>
      ...
      >
        {data?.results.map((popularMovie, i) => {
          return <PopularMovies popularMovie={popularMovie} key={i} />;
        })}
      </Grid2>
