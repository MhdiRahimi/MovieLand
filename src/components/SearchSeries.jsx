...
const SearchSeries = ({ pseries, mod }) => {
  ...
  const detailFetch = async () => {
    const res = await axios.get(
      `https://api.themoviedb.org/3/tv/${pseries?.id}?api_key=${process.env.REACT_APP_KEY}&language=en-US&page=1`
    );
    setData(res.data);
  };
    ...
    <>
      {data ? (
        <>
          ...
          <Recomended movie_id={pseries?.id} mod={mod} />
        </>
      ) : (
        'not found'
      )}
